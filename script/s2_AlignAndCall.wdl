...
task M2 {
  ...
  command <<<
      ...
      /gatk-4.2.0.0/gatk --java-options "-Xmx~{command_mem}m" Mutect2 \
        -R ~{ref_fasta} \
        -I ~{input_bam} \
...
task Filter {
  ...
  command <<<
      ...
      /gatk-4.2.0.0/gatk --java-options "-Xmx2500m" FilterMutectCalls -V ~{raw_vcf} \
        -R ~{ref_fasta} \
        -O filtered.vcf \
        --stats ~{raw_vcf_stats} \
      ...
      /gatk-4.2.0.0/gatk VariantFiltration -V filtered.vcf \
        -O ~{output_vcf} \
...
task MergeStats {
  ...
  command{
    ...
    /gatk-4.2.0.0/gatk MergeMutectStats --stats ~{shifted_stats} --stats ~{non_shifted_stats} -O raw.combined.stats
  }
  output {
...
task SplitMultiAllelicsAndRemoveNonPassSites {
  ...
  command {
    ...
    /gatk-4.2.0.0/gatk LeftAlignAndTrimVariants \
      -R ~{ref_fasta} \
      -V ~{filtered_vcf} \
      -O split.vcf \
...
task FilterNuMTs {
  ...
  command {
    ...
    /gatk-4.2.0.0/gatk NuMTFilterTool \
      -R ~{ref_fasta} \
      -V ~{filtered_vcf} \
      -O ~{output_vcf} \
...
task FilterLowHetSites {
  ...
  command {
    ...
    /gatk-4.2.0.0/gatk MTLowHeteroplasmyFilterTool \
      -R ~{ref_fasta} \
      -V ~{filtered_vcf} \
      -O ~{output_vcf} \
